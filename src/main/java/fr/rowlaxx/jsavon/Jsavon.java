...
public abstract class Jsavon implements Serializable {
	...
	private static final Map<Class<? extends Jsavon>, Exception> valid = new HashMap<>();
	static final Converter converter = ConverterFactory
			.newDefaultInstance()
			.build();
	
	public static final boolean isVerified(Class<? extends Jsavon> clazz) {
		return valid.containsKey(clazz);
	}

	public static final boolean isValid(Class<? extends Jsavon> clazz) {
		if (!isVerified(clazz))
			try{
				verify(clazz);
	...
	private static final void verify(Class<? extends Jsavon> clazz) {
